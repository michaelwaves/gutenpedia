...
as much as possible, we try to use classes/types from the `backend/inference/neuronpedia-inference-client`, which is autogenerated from the openapi spec under `openapi/schemas/openapi/inference-server.yaml`
